...
APP_HOME="$(cygpath -w "$REPO/modules/microproject_ui/build/install/microproject_ui")"
JH="$(cygpath -w "$(java -XshowSettings:properties -version 2>&1 | grep 'java.home' | sed 's/.*= *//')")"
JAVA_EXE="$JH\\bin\\java.exe"
CLASSPATH="$APP_HOME\\lib\\microproject_ui.jar;$APP_HOME\\lib\\jgoodies-forms-1.9.0.jar;$APP_HOME\\lib\\*"
