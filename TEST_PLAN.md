...
- GUI-NC-03/08: `TaskTableGanttGridGuiAcceptanceTest.twentyMixedTasksRemainAccessibleAfterMouseScroll` を追加し、FSで連続する10タスク＋独立10タスク（計20タスク）を生成した実GUIで、20行の存在と縦スクロールバーのマウス操作を確認した。`./gradlew.bat :micrproject_ui:guiTest --tests "com.microproject.pm.graphic.views.TaskTableGanttGridGuiAcceptanceTest" --console=plain`（BUILD SUCCESSFUL）。
