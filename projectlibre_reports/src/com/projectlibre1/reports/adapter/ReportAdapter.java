		...
		while(iterator.hasNext()) {
			...
			if(isAggregable(field)) {
				...
				if(field.isMoney()) {
					...
					expression.setText("com.projectlibre1.datatype.DurationFormat.formatWork($V{" + getFieldName(field, true) + "Sum})" );
				} else if(field.isDuration()) {
					expression.setValueClass(String.class);
					expression.setText("com.projectlibre1.datatype.DurationFormat.format($V{" + getFieldName(field, true) + "Sum})" );
				}
