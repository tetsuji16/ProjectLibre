...
"%JAVA_HOME%\bin\javac" --release 21 -cp "%CLASSPATH%" -d projectlibre_build\build projectlibre_ui\src\com\projectlibre1\pm\graphic\frames\StartupFactory.java
if errorlevel 1 echo Failed to compile StartupFactory.java

echo Compiling Init.java...
"%JAVA_HOME%\bin\javac" --release 21 -cp "%CLASSPATH%" -d projectlibre_build\build projectlibre_core\src\com\projectlibre1\init\Init.java
