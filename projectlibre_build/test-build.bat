...
echo Compiling StartupFactory.java with --release 8...
"%JAVA_HOME%\bin\javac" --release 8 -cp "%CP%" -d "%BUILD_DIR%" "%SOURCE_DIR%\projectlibre_ui\src\com\projectlibre1\pm\graphic\frames\StartupFactory.java" 2>&1

echo Compiling Init.java with --release 8...
"%JAVA_HOME%\bin\javac" --release 8 -cp "%CP%" -d "%BUILD_DIR%" "%SOURCE_DIR%\projectlibre_core\src\com\projectlibre1\init\Init.java" 2>&1
