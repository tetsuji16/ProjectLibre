...
| Hierarchy preorder indexing | `AbstractMutableNodeHierarchy.getIndexOfNode` | Replaced the raw child collection/iterator with `List<?>` and enhanced-for traversal. A focused real-hierarchy test exposed and fixed swapped recursive arguments that made descendants return `-1`; it now confirms root, parent, child, and sibling preorder indexes. Focused and full core tests plus downstream compilation passed. |
