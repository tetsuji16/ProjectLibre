...
| Hierarchy preorder indexing and traversal | `AbstractMutableNodeHierarchy.getIndexOfNode`, `visitAll`, `visitAllLevelOrder`, `visitLeaves` | Replaced raw child collections/iterators/enumeration with `List<?>`, `Enumeration<?>`, and enhanced-for traversal; removed redundant iterator reinitialization without changing visitation phases. A focused real-hierarchy test exposed and fixed swapped recursive arguments that made descendants return `-1`; tests verify root/parent/child/sibling preorder indexes plus preorder and level-order visitation sequences. Focused and full core tests plus downstream compilation passed. |
