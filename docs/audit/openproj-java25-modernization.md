...
| Node dirty-state delegation | `NodeBridge.isDirty`, `setDirty` | Confirmed both type-check/cast hunks match OpenProj. Used pattern bindings; added a headless regression for delegated DataObject state and the unchanged false/no-op behavior for non-DataObject implementations. Focused `NodeBridgeDirtyStateTest`, full `:microproject_core:test`, and application/exchange/UI/reports `compileJava` passed. |
