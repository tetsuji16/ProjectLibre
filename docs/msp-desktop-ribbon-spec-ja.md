...
3. `File` は通常のリボン・バンドではなく Backstage である。New / Open / Info / Save /
   ...
   したがって、通常タブ向けの幅縮退処理で `File` を一個の「ファイル …」ランチャーに
   置き換えてはならない。
...
5. 幅が不足したときは、各グループを定義済みの表示段階に従って縮小し、最後に**グループ単位**の
   ドロップダウンへ畳む。タブ全体を一個のランチャーへ置換してはならない。すべてのグループが
   最小表示になっても収まらない場合は、左右のスクロール操作でグループ列を移動する。
...
これは Windows 版 Office の Fluent Ribbon の挙動を基準にする。ウィンドウを狭めても、
全体を一律に小さくしたり、タブ面を一個のメニューに置換したりしない。グループごとに、
次の順序で表示を変える。

1. ギャラリー又は低優先度の候補数を減らす。
2. 大きいボタンを中・小ボタンへ変え、複数列のコマンドを再配置する。
3. そのグループを、代表アイコン・グループ名・ドロップダウンを持つ一個の**グループ代理**へ畳む。
   代理メニューには、そのグループの全コマンドを元の Action ID のまま載せる。
4. 全グループが代理表示でも横幅を超える場合、グループ列を左詰めのまま水平スクロールする。
   左右端のスクロール操作は、隠れたグループの存在と移動方向を示す。

`最小化されたリボン`（タブ行だけを表示し、タブ選択時に一時展開する利用者操作）と、
狭幅の自動縮退は別の状態である。前者以外で、標準タブのコマンド面をタブ全体ランチャー一個へ
縮退してはならない。

縮退の入力は DPI 前の論理ピクセルの固定閾値ではなく、タイトルバー、QAT、検索欄、境界を除いた
実際のリボン・クライアント幅と、各グループの現在の表示段階の実測幅とする。DPI、ロケール、
フォント又はウィンドウ装飾の変更では再測定する。

### 2.2 左寄せと余白

グループ列は常に左詰めで、余剰幅を得るためにコマンドをバンドの中央へ移動してはならない。
ただし Office と同様に、少数グループのタブでは右端の未使用領域が残ること自体は正しい。
従来の「右側空白が30%を超えたら無条件にNG」という判定は廃止する。

代わりに、次をNGとする。

- 標準タブが一個のタブ全体ランチャーだけになること。
- グループ又はその代理が中央揃えになること。
- 収まらないグループを無表示、クリップ、又はタブ全体メニューへ退避すること。
- 同じ幅で再レイアウトしたとき、グループ順・Action ID・有効状態が変わること。

### 2.3 応答リボンの受入基準

- `File` は Backstage を開き、通常バンド又は「ファイル …」縮退ランチャーを表示しない。
- 標準タブでは、各表示幅で少なくとも一つ以上の直接グループ又はグループ代理が左端から順番に
  見え、タブ全体代理は存在しない。
- グループ代理を Robot で開くと、そのグループに属する全コマンドが元の Action ID で到達できる。
- 日本語・英語、100%・125%・150%、および各グループの表示段階が遷移する前後の幅で、直接ボタン、
  グループ代理、スクロール操作の境界・非重複・クリック可能性を検証する。
- 視覚テストは描画ピクセルの有無や外側バンドの幅だけで合格にしてはならない。アクティブな
  グループ数、代理の有無、コマンド/代理の矩形、左寄せ、クリップなしを構造的に検証し、Robotの
  スクリーンショットで照合する。
