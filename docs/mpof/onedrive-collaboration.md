...
OneDrive sync client. Put the `.podx` file and its adjacent
...
Each save also uses a stable `<project>.podx.lock` sidecar as a short-lived
...
processes before the atomic replacement of the `.podx` file.
...
merge their podx task-operation logs, so both edits survive the later save.

When a task with a local lease changes externally, the application does not
overwrite the shared podx. It offers **Save Copy** or **Cancel** so that both
...
operation-log merge is limited to synchronized podx files and deterministic
