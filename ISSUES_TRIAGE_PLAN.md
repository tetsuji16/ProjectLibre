...
| #47 | Copy/Cut-Paste does not work well | **Investigated 2026-08-19: root cause = double-wiring, not just missing shortcuts.** `SpreadSheet` ctor calls `NodeListTransferHandler.registerWith(this)` (line 146), which `setTransferHandler(...)` and makes Swing auto-register `ctrl C/X/V` on the component's `WHEN_FOCUSED` input map (standard TransferHandler paste). That coexists with `GraphicManager.applyMicrosoftShortcuts` wiring `ACTION_PASTE/COPY/CUT` to `ctrl V/C/X` on the root-pane `WHEN_IN_FOCUSED_WINDOW`. The two layers both answer `ctrl V`, so paste is non-deterministic / "doesn't work well". NOTE: AGENTS.md claims "Removed the NodeListTransferHandler … duplicate registrations" — but `registerWith` is still present, so that hardening was NOT actually applied to `SpreadSheet`. `NodeListTransferHandler` also owns the real paste logic (`importData`), so it cannot be deleted outright; the fix is to stop the keyboard double-wiring, not remove the class. **Fix plan (pending sign-off):** drop `registerWith` from `SpreadSheet` and route paste through `ACTION_PASTE` only (TransferHandler.importData is still reachable via `getPasteAction()`); then re-run `MicrosoftShortcutsRootPaneTest` + `NodeListTransferablePasteFailureTest`. |
